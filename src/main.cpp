...
void printData();
void MelhorInsercao(vector<int> &solucao, int escolhido,
...
void InsercaoMaisBarata(vector<int> &conjuntoDeLocais, vector<int> &solucao,
                        vector<tLocais> &melhorCaminho);
void Limitar_Variacoes_Dos_Indices(int &indiceInicial, int &indiceFinal);
void EscreverResultadosNosArquivos(fstream &File, int *counters,
                                   int iteracoesMaxima, char *Nomearquivo);
void VerificarLimitesParaIndices(int &indiceInicial1, int &indiceFinal1,
                                 int &indiceInicial2, int &indiceFinal2);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < iteracoesMaxima; i++) {
    ...
    while (!conjuntoDeLocais.empty()) {
      ...
      solucao.emplace(solucao.begin() +
                          melhorCaminho[escolhaAleatoriaDoVertice].i,
  ...
  fileSwap.open("/home/rodrigo/Documents/TSP/resultadosInstancias/Swap.txt",
                ios::app);
  EscreverResultadosNosArquivos(fileSwap, counterSwaps, iteracoesMaxima,
                                argv[1]);
  fileSwap.close();

  fileReinsertion.open(
      "/home/rodrigo/Documents/TSP/resultadosInstancias/Reinsertion.txt",
      ios::app);
  EscreverResultadosNosArquivos(fileReinsertion, counterReinsertion,
                                iteracoesMaxima, argv[1]);
  fileReinsertion.close();

  fileReinsertion_3.open(
      "/home/rodrigo/Documents/TSP/resultadosInstancias/Reinsertion_3.txt",
      ios::app);
  EscreverResultadosNosArquivos(fileReinsertion_3, counterReinsertion_3,
                                iteracoesMaxima, argv[1]);
  fileReinsertion_3.close();

  fileReinsertion_2.open(
      "/home/rodrigo/Documents/TSP/resultadosInstancias/Reinsertion_2.txt",
      ios::app);
  EscreverResultadosNosArquivos(fileReinsertion_2, counterReinsertion_2,
                                iteracoesMaxima, argv[1]);
  fileReinsertion_2.close();

  fileTwo_Opt.open(
      "/home/rodrigo/Documents/TSP/resultadosInstancias/Two_opt.txt", ios::app);
  EscreverResultadosNosArquivos(fileTwo_Opt, counterTwo_Opt, iteracoesMaxima,
                                argv[1]);
  fileTwo_Opt.close();

  fileDoubleBridge.open(
      "/home/rodrigo/Documents/TSP/resultadosInstancias/DoubleBridge.txt",
      ios::app);
  EscreverResultadosNosArquivos(fileDoubleBridge, counterDoubleBridge,
                                iteracoesMaxima, argv[1]);
  ...
  file.open(
      "/home/rodrigo/Documents/Projeto/TSP/TSP/resultadosInstancias/rat99.txt",
      ios::app);
  ...
  file << endl << "Distancia = " << custoFinal << endl;
...
int GenerateRandomNumber(int tamanho) { return (rand() % tamanho) + 1; }

bool Ordena(tLocais a, tLocais b) { return a.distancia < b.distancia; }
  ...
  for (int i = 1; i < quantidadeDeIteracoes; i++) {
    ...
    for (int j = i + tamanho; j < quantidadeDeIteracoes; j++) {
      double CustoTotalDeReinsercao = CustoTotalDeTirarArcosIniciais -
                                      matrizAdj[solucao[j]][solucao[j + 1]];
      CustoTotalDeReinsercao += matrizAdj[solucao[j]][solucao[i]] +
...
void VerificarLimitesParaIndices(int &indiceInicial1, int &indiceFinal1,
  ...
  if (indiceInicial1 <= indiceInicial2 && indiceInicial2 <= indiceFinal1) {
    ...
    if (indiceInicial2 +
      ...
      int novoIndice2 =
          indiceInicial2 - quantidadeDeVerticesEntreIndiceInicial1_IndiceFinal2;
